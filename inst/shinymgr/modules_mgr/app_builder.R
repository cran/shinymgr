...
app_builder_server <- function(id, shinyMgrPath) {
  moduleServer(id, function(input, output, session) {
    ...
    output$instructions <- renderText(
      ...
      planning your App, see the walkthrough in the tutorials. Once your app is planned,
      this builder allows will document the details of your app in the 
      SQLite database, including the App's metadata, tabs,
      mods, and how the mods will be stitched together. The builder than writes 
      a script for your App to the modules_app directory. Your App can then be 
        ...
        tagList(
          sidebarLayout(
            sidebarPanel(
              width = 6,
              h1("Your App"),
              reactable::reactableOutput(ns("app_summary")),
              h6("")
            ),
            mainPanel(
              width = 6,
    ...
    output$mod_info <- reactable::renderReactable({
      reactable::reactable(
        data = mod_info,
        filterable = TRUE,
        searchable = TRUE
      ...
      if (data2$tab_name() %in% c(tabNames, tabs_2_add()$pkTabName)) {
        ...
        insertUI(
          ...
          ui = tagList(
            ...
            reactable::reactableOutput(ns('mod_info'))
          )
        )
    ...
    output$app_summary <- reactable::renderReactable({
      reactable::reactable(data = app_meta())
    })
    ...
    observeEvent(input$finish_app, {
      ...
      output$mod_arg_table <- reactable::renderReactable({
        reactable::reactable(
      ...
      output$mod_return_table <- reactable::renderReactable({
        reactable::reactable(
      ...
      insertUI(
        ...
        ui = tagList(
          ...
          reactable::reactableOutput(ns('mod_arg_table')),
          htmlOutput(ns('mod_return_table_text')),
          reactable::reactableOutput(ns('mod_return_table'))
        )
      )
    ...
    observeEvent(needs_reset$reset, { 
      ...
      if (needs_reset$reset()) {
        ...
        shinyjs::delay(50, {shinyjs::hide('start')})
      }
    })
