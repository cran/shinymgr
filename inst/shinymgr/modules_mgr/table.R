table_ui <- function(id) {
  tagList(
    ...
    br(),
    reactable::reactableOutput(outputId = NS(id, id = "show_table"))
  ) #end taglist
} #end ui function
...
table_server <- function(id, con) {
  moduleServer(
    ...
    function(input, output, session) {
      ...
      #create reactable--------------------
      output$show_table <- reactable::renderReactable({
        reactable::reactable(
          db_table(),
          selection = "single",
          filterable = TRUE,
          details = function(i) {
            if (nrow(shinyTable) != 0) {
              if (shinyTable$subTable != "" & !is.na(shinyTable$subTable)) {
                ...
                #render reactable
                output[[paste0("subTable_", i)]] <- reactable::renderReactable({
                  reactable::reactable(
                    subTable()[subTable()[[
                ...
                reactable::reactableOutput(NS(id, paste0("subTable_", i)))
              }
            } else {
              NULL
            }
      ...
      observeEvent(
        ...
        handlerExpr = {
          #get selected row
          rowIndex <- reactable::getReactableState(outputId = "show_table", name = "selected")
        ...
        handlerExpr = {
          ...
          tryCatch(
            expr = {
              ...
              if (id == "apps") {
                deleted <- deleted + DBI::dbExecute(conn = con(), statement = deleteTabStatement)
              }
              deleted <- deleted + DBI::dbExecute(conn = con(), statement = deleteStatement)
